//! COPY method implementation (RFC 4918 Section 9.8).
...
pub async fn handle_copy(
    ...
    if source_trimmed.is_empty() || dest_trimmed.is_empty() {
        return Ok(Response::builder()
            ...
            .body("Cannot copy root collection".to_string())
    ...
    match folder_result {
        Ok(folder) => {
            folder_service
                ...
                .await
                .map_err(|e| AppError::internal(format!("Folder copy failed: {}", e)))?;
        }
        Err(_) => {
            ...
            file_service
                ...
                .await
                .map_err(|e| AppError::internal(format!("File copy failed: {}", e)))?;
        }
    }

    tracing::info!("COPY completed: '{}' → '{}'", source_path, dest_path);
